...
window.Helpers.switchImage(u),window.Helpers.setTheme(window.Helpers.getPreferredTheme()),window.matchMedia("(prefers-color-scheme: dark)").addEventListener("change",()=>{const e=window.Helpers.getStoredTheme();e!=="light"&&e!=="dark"&&window.Helpers.setTheme(window.Helpers.getPreferredTheme())});function m(){const e=window.innerWidth-document.documentElement.clientWidth;document.body.style.setProperty("--bs-scrollbar-width",`${e}px`)}m(),window.addEventListener("DOMContentLoaded",()=>{window.Helpers.showActiveTheme(window.Helpers.getPreferredTheme()),m(),window.Helpers.initSidebarToggle(),document.querySelectorAll("[data-bs-theme-value]").forEach(e=>{e.addEventListener("click",()=>{const o=e.getAttribute("data-bs-theme-value");window.Helpers.setStoredTheme(templateName,o),window.Helpers.setTheme(o),window.Helpers.showActiveTheme(o,!0),window.Helpers.syncCustomOptions(o);let i=o;o==="system"&&(i=window.matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light");const r=document.querySelector(".template-customizer-semiDark");r&&(o==="dark"?r.classList.add("d-none"):r.classList.remove("d-none")),window.Helpers.switchImage(i)})})});let p=document.getElementsByClassName("dropdown-language");if(p.length){let i=function(r){document.documentElement.setAttribute("dir",r),r==="rtl"?localStorage.getItem("templateCustomizer-"+templateName+"--Rtl")!=="true"&&window.templateCustomizer&&window.templateCustomizer.setRtl(!0):localStorage.getItem("templateCustomizer-"+templateName+"--Rtl")==="true"&&window.templateCustomizer&&window.templateCustomizer.setRtl(!1)};var x=i;let e=p[0].querySelectorAll(".dropdown-item");const o=p[0].querySelector(".dropdown-item.active");i(o.dataset.textDirection);for(let r=0;r<e.length;r++)e[r].addEventListener("click",function(){let S=this.getAttribute("data-text-direction");window.templateCustomizer.setLang(this.getAttribute("data-language")),i(S)})}setTimeout(function(){let e=document.querySelector(".template-customizer-reset-btn");e&&(e.onclick=function(){window.location.href=baseUrl+"lang/en"})},1500);const g=document.querySelector(".dropdown-notifications-all"),w=document.querySelectorAll(".dropdown-notifications-read");g&&g.addEventListener("click",e=>{w.forEach(o=>{o.closest(".dropdown-notifications-item").classList.add("marked-as-read")})}),w&&w.forEach(e=>{e.addEventListener("click",o=>{e.closest(".dropdown-notifications-item").classList.toggle("marked-as-read")})}),document.querySelectorAll(".dropdown-notifications-archive").forEach(e=>{e.addEventListener("click",o=>{e.closest(".dropdown-notifications-item").remove()})}),[].slice.call(document.querySelectorAll('[data-bs-toggle="tooltip"]')).map(function(e){return new bootstrap.Tooltip(e)});const y=function(e){e.type=="show.bs.collapse"||e.type=="show.bs.collapse"?e.target.closest(".accordion-item").classList.add("active"):e.target.closest(".accordion-item").classList.remove("active")};[].slice.call(document.querySelectorAll(".accordion")).map(function(e){e.addEventListener("show.bs.collapse",y),e.addEventListener("hide.bs.collapse",y)}),window.Helpers.setAutoUpdate(!0),window.Helpers.initPasswordToggle(),window.Helpers.initSpeechToText(),window.Helpers.initNavbarDropdownScrollbar();let b=document.querySelector("[data-template^='horizontal-menu']");if(b&&(window.innerWidth<window.Helpers.LAYOUT_BREAKPOINT?window.Helpers.setNavbarFixed("fixed"):window.Helpers.setNavbarFixed("")),window.addEventListener("resize",function(e){b&&(window.innerWidth<window.Helpers.LAYOUT_BREAKPOINT?window.Helpers.setNavbarFixed("fixed"):window.Helpers.setNavbarFixed(""),setTimeout(function(){window.innerWidth<window.Helpers.LAYOUT_BREAKPOINT?document.getElementById("layout-menu")&&document.getElementById("layout-menu").classList.contains("menu-horizontal")&&f.switchMenu("vertical"):document.getElementById("layout-menu")&&document.getElementById("layout-menu").classList.contains("menu-vertical")&&f.switchMenu("horizontal")},100))},!0),!(v||window.Helpers.isSmallScreen())&&(typeof window.templateCustomizer<"u"&&(window.templateCustomizer.settings.defaultMenuCollapsed?window.Helpers.setCollapsed(!0,!1):window.Helpers.setCollapsed(!1,!1)),typeof config<"u"&&config.enableMenuLocalStorage))try{localStorage.getItem("templateCustomizer-"+templateName+"--LayoutCollapsed")!==null&&window.Helpers.setCollapsed(localStorage.getItem("templateCustomizer-"+templateName+"--LayoutCollapsed")==="true",!1)}catch{}})();const C={container:"#autocomplete",placeholder:"Buscar [CTRL + K]",classNames:{detachedContainer:"d-flex flex-column",detachedFormContainer:"d-flex align-items-center justify-content-between border-bottom",form:"d-flex align-items-center",input:"search-control border-none",detachedCancelButton:"btn-search-close",panel:"flex-grow content-wrapper overflow-y-auto position-relative",panelLayout:"h-100",clearButton:"d-none",item:"d-block"}};let d={};function L(){fetch(baseUrl+"search/data",{headers:{"X-Requested-With":"XMLHttpRequest",Accept:"application/json"}}).then(a=>{if(!a.ok)throw new Error("Failed to fetch search data");return a.json()}).then(a=>{d=a,H()}).catch(a=>console.error("Error loading search data:",a))}function H(){if(document.getElementById("autocomplete"))return autocomplete({...C,openOnFocus:!0,onStateChange({state:l,setQuery:c}){if(l.isOpen){document.body.style.overflow="hidden",document.body.style.paddingRight="var(--bs-scrollbar-width)";const t=document.querySelector(".aa-DetachedCancelButton");if(t&&(t.innerHTML='<span class="text-body-secondary">[esc]</span> <span class="icon-base icon-md ti tabler-x text-heading"></span>'),!window.autoCompletePS){const n=document.querySelector(".aa-Panel");n&&(window.autoCompletePS=new PerfectScrollbar(n))}}else l.status==="idle"&&l.query&&c(""),document.body.style.overflow="auto",document.body.style.paddingRight=""},render(l,c){var m;const{render:t,html:n,children:s,state:u}=l;if(!u.query){const p=n`
        ...
        `;t(p,c);return}if(!l.sections.length){t(n`
          ...
          `,c);return}t(s,c),(m=window.autoCompletePS)==null||m.update()},getSources(){const l=[];if(d.navigation){const c=Object.keys(d.navigation).filter(t=>t!=="files"&&t!=="members").map(t=>({sourceId:`nav-${t}`,getItems({query:n}){const s=d.navigation[t];return n?s.filter(u=>u.name.toLowerCase().includes(n.toLowerCase())):s},getItemUrl({item:n}){return baseUrl+n.url},templates:{header({items:n,html:s}){return n.length===0?null:s`<span class="search-headings">${t}</span>`},item({item:n,html:s}){return s`
                ...
                `}}}),d.navigation.members&&l.push({sourceId:"members",getItems({query:t}){const n=d.navigation.members;return t?n.filter(s=>s.name.toLowerCase().includes(t.toLowerCase())):n},getItemUrl({item:t}){return baseUrl+t.url},templates:{header({items:t,html:n}){return t.length===0?null:n`<span class="search-headings">Members</span>`},item({item:t,html:n}){return n`
                ...
                `}}})}return l}})}document.addEventListener("keydown",a=>{(a.ctrlKey||a.metaKey)&&a.key==="k"&&(a.preventDefault(),document.querySelector(".aa-DetachedSearchButton").click())});document.documentElement.querySelector("#autocomplete")&&L();
