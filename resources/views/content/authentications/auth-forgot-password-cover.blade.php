...
<style>
  ...
  :root {
    --gov-navy:#001a4d;--gov-blue:#003087;--gov-mid:#0047b3;
    --gold:#c9a227;--gold-light:#e8c547;--gold-pale:#f5e6b0;
    --cream:#fafaf7;--text-dark:#0d1b2a;--text-muted:#5a6a7a;
  ...
  *{box-sizing:border-box}
  ...
  .left-geo .geo-line:nth-child(2){top:40%;animation-delay:1s;opacity:.6}
  .left-geo .geo-line:nth-child(3){top:65%;animation-delay:2s}
  .left-geo .geo-line:nth-child(4){top:85%;animation-delay:1.5s;opacity:.4}
  ...
  /* Ícono central en panel izquierdo */
  ...
  .left-title{font-family:'Playfair Display',Georgia,serif;font-size:clamp(1.8rem,3vw,2.5rem);font-weight:900;line-height:1.1;margin-bottom:.75rem;letter-spacing:-.02em;color:var(--white)}
  ...
  .left-step-list{text-align:left;list-style:none;padding:0;margin:0;background:rgba(255,255,255,.04);border:1px solid rgba(201,162,39,.18);border-radius:14px;overflow:hidden}
  .left-step-list li{display:flex;align-items:flex-start;gap:.85rem;padding:1rem 1.25rem;border-bottom:1px solid rgba(201,162,39,.1)}
  .left-step-list li:last-child{border-bottom:none}
  .step-num{width:26px;height:26px;background:linear-gradient(135deg,var(--gold),var(--gold-light));border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:.7rem;font-weight:700;color:var(--gov-navy);flex-shrink:0;margin-top:1px}
  .step-text strong{display:block;font-size:.82rem;font-weight:600;color:rgba(255,255,255,.85);margin-bottom:.15rem}
  .step-text span{font-size:.74rem;color:rgba(255,255,255,.4)}
  ...
  .form-brand-name{font-family:'Playfair Display',serif;font-size:1.3rem;font-weight:700;color:var(--gov-navy);letter-spacing:-.02em}
  ...
  .form-subtitle{font-size:.82rem;color:var(--text-muted);font-weight:400;line-height:1.5;margin-bottom:0}
  ...
  .pulso-field{margin-bottom:1.25rem}
  ...
  .pulso-input-icon{position:absolute;left:1rem;top:50%;transform:translateY(-50%);color:#94a3b8;font-size:1rem;pointer-events:none;transition:color .2s;z-index:1}
  .pulso-input{width:100%;height:48px;padding:0 1rem 0 2.75rem;border:1.5px solid var(--border);border-radius:10px;font-size:.9rem;font-family:'DM Sans',sans-serif;color:var(--text-dark);background:var(--white);transition:all .2s ease;outline:none;appearance:none}
  .pulso-input::placeholder{color:#b8c3cc}
  .pulso-input:focus{border-color:var(--gov-blue);box-shadow:0 0 0 3px rgba(0,48,135,.1)}
  .pulso-input:focus~.pulso-input-icon,.pulso-input-wrap:focus-within .pulso-input-icon{color:var(--gov-blue)}
  ...
  .btn-pulso-primary{width:100%;height:50px;background:linear-gradient(135deg,var(--gov-navy) 0%,var(--gov-blue) 100%);color:white;border:none;border-radius:10px;font-family:'DM Sans',sans-serif;font-size:.9rem;font-weight:600;letter-spacing:.04em;cursor:pointer;position:relative;overflow:hidden;transition:all .3s ease;display:flex;align-items:center;justify-content:center;gap:.6rem;box-shadow:0 4px 20px rgba(0,48,135,.35)}
  .btn-pulso-primary::before{content:'';position:absolute;inset:0;background:linear-gradient(135deg,transparent 0%,rgba(201,162,39,.2) 100%);opacity:0;transition:opacity .3s}
  ...
  .back-link{display:flex;align-items:center;justify-content:center;gap:.4rem;margin-top:1.5rem;color:var(--text-muted);text-decoration:none;font-size:.84rem;font-weight:500;transition:color .2s}
...
<div class="pulso-login-wrapper">
  ...
  <div class="pulso-left">
    ...
    <div class="left-content">
      ...
      <p class="left-pretitle">Seguridad de Acceso &bull; {{ $ci?->sigla ?? 'PULSO UGEL' }}</p>
      <h1 class="left-title">Recuperar<br><span>Acceso</span></h1>
      ...
      <ul class="left-step-list">
        <li>
          <span class="step-num">1</span>
          <div class="step-text">
            <strong>Ingresa tu correo institucional</strong>
            <span>El correo con el que te registraste en el sistema</span>
          </div>
        </li>
        <li>
          ...
          <div class="step-text">
            ...
            <span>Te enviaremos un enlace seguro con vigencia de 60 minutos</span>
          </div>
        </li>
        <li>
          <span class="step-num">3</span>
          <div class="step-text">
            <strong>Establece una nueva contraseña</strong>
            <span>Crea una contraseña segura y accede al sistema</span>
          </div>
        </li>
      </ul>
    </div>
  ...
  <div class="pulso-right">
    <div class="form-container">
      ...
      <div class="form-header">
        ...
        <h2 class="form-title">
          <span class="form-title-accent"></span>Recuperar contraseña
        </h2>
        @php $dominio = $ci?->correo_institucional ? explode('@', $ci->correo_institucional)[1] ?? null : null; @endphp
        <p class="form-subtitle">
          Ingresa tu correo{{ $dominio ? ' institucional (@' . $dominio . ')' : '' }} y te enviaremos las instrucciones
        </p>
      </div>
      ...
      <form id="formAuthentication" action="{{ route('password.email') }}" method="POST" novalidate>
        @csrf
